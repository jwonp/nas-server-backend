...
def delete_selected_files(selected_list):
    for selected in selected_list:
        file = File.objects.get(file_name=selected.get('file_name'), file_path=selected.get(
            'file_path'), file_owner=selected.get('file_owner'))
        file.delete()
...
def check_remaining_storage_space(upload_files, username):
    ...
    used_storage_size = UsedStorageSizeSerializer(
        user_storage, many=False).data.get('used_storage_size')
...
def delete_file(delete_files, username, saved_path):
    ...
    for file_name in delete_files:
        ...
        if ("folder:" in name):
            ...
            file_size = File.objects.filter(
                file_owner=username, file_path__startswith=f'{file_path}{name}/').aggregate(Sum('file_size')).get('file_size__sum')
        else:
    ...
    for item in meta_data:
        ...
        if item.get('is_folder') == True:
            ...
            files = File.objects.filter(
                file_owner=username, file_path__startswith=f'{path}{name}/')
            for file in files:
                if file:
                    file.delete()
            folders = Folder.objects.filter(
                owner=username, base_path__startswith=f'{path}{name}/')
            for folder in folders:
                if folder:
                    folder.delete()
            # 목표 폴더 삭제
            folder = Folder.objects.get(
                owner=username, folder_name=f'folder:{name}', base_path=path)
            if folder:
                folder.delete()
            file = File.objects.get(
                file_owner=username, file_name=f'folder:{name}', file_path=path)
            if file:
                file.delete()
        else:
            file = File.objects.get(
                file_owner=username, file_name=name, file_path=path)
            if file:
                file.delete()
    return meta_set
